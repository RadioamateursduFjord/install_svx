...
# Vérification de la version Debian (Trixie = 13)
if [[ -r /etc/os-release ]]; then
  . /etc/os-release
  if [[ "${ID:-}" != "debian" || "${VERSION_ID:-}" != "13" ]]; then
    echo "Attention: ce script est prévu pour Debian 13 Trixie, pas pour ${PRETTY_NAME:-inconnu}." >&2
    echo "Appuie sur Ctrl+C pour annuler si ce n'est pas volontaire." >&2
    sleep 5
  fi
fi
...
User=svxlink
Group=svxlink
# Si besoin de capacités GPIO / réseau, tu peux ajouter:
# CapabilityBoundingSet=CAP_NET_BIND_SERVICE CAP_NET_ADMIN CAP_SYS_NICE
